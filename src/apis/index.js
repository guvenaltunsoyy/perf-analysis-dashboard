import {GET_DOM_EVENTS_URI, GET_PAINTS_URI, GET_RESOURCES_URI, GET_TTFB_URI, GET_WINDOW_EVENTS_URI} from "../constants";
...
export const getDomEvents = async (startDate = new Date(), endDate = new Date()) => {
    const response = await getData(`${GET_DOM_EVENTS_URI}?startDate=${startDate?.toUTCString()}&endDate=${endDate?.toUTCString()}`).then(checkStatus).catch(err => console.log(err));
    return response.data;
}
export const getWindowEvents = async (startDate = new Date(), endDate = new Date()) => {
    const response = await getData(`${GET_WINDOW_EVENTS_URI}?startDate=${startDate?.toUTCString()}&endDate=${endDate?.toUTCString()}`).then(checkStatus).catch(err => console.log(err));
    return response.data;
}